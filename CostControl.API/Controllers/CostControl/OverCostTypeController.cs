namespace CostControl.API.Controllers.CostControl
{
	...
	public class OverCostTypeController : BaseApiController<OverCostType, OverCostTypeLogic, byte>
	{
		...
		public IEnumerable<dynamic> GetExternalData()
		{
			try
			{
				return (PDKBusinessLogic as OverCostTypeLogic).GetExternalData();
			}
			catch (Exception e)
			{
		...
		public bool AddExternalData(string id)
		{
			try
			{
				(PDKBusinessLogic as OverCostTypeLogic).AddExternalData(id);
				return true;
			}
			catch (Exception e)
			{
