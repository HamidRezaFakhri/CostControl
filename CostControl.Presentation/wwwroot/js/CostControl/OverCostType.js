...
var middleUrl = serviceUrl + "OverCostType/";
...
function trdblclick(id) {
    if (id !== undefined && id >= 0) {
        editEntityModal(id, '/OverCostType/EditOverCostType/', 'گروه مرکز هزینه');
    }
}
            ...
            if (key === "data") {
                $.each(value, function (i, val) {
                    var item = '<tr id="tr_' + val.id + '" ondblclick="trdblclick(' + val.id + ')">' +
                        ...
                        '<td>' +
                        '<a class="btn btn-sm btn-warning" data-id=\'' + val.id + '\' onClick="editEntityModal(' + val.id + ', \'/OverCostType/EditOverCostType/\'' + ', \'گروه مرکز هزینه\'' + ')"> <i class="fa fa-edit"> ویرایش </i> </a>' +
                        '<span> | </span>' +
                        '<a class="btn btn-sm btn-danger" data-id=\'' + val.id + '\' onClick="deleteEntityModal(' + val.id + ', \'گروه مرکز هزینه\'' + ', \'' + val.name + '\'' + ', deleteOverCostType' + ')"> <i class="fa fa-trash"> حذف </i> </a>' +
                        '</td>' +
                        '</tr>';
                    $("#setOverCostTypeList").append(item);
                    $("#LoadingStatus").html(" ");
                });
            }
...
function deleteOverCostType(id) {
