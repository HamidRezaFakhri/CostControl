...
function trdblclick(id) {
    if (id !== undefined && id >= 0) {
        editEntityModal(id, '/Menu/EditMenu/', 'دستور غذا');
    }
}
            ...
            if (key === "data") {
                $.each(value, function (i, val) {
                    var item = '<tr id="tr_' + val.id + '" ondblclick="trdblclick(' + val.id + ')">' +
                        ...
                        '<td>' +
                        '<a class="btn btn-sm btn-warning" data-id=\'' + val.id + '\' onClick="editEntityModal(' + val.id + ', \'/Menu/EditMenu/\'' + ', \'دستور غذا\'' + ')"> <i class="fa fa-edit"> ویرایش </i> </a>' +
                        '<span> | </span>' +
                        '<a class="btn btn-sm btn-danger" data-id=\'' + val.id + '\' onClick="deleteEntityModal(' + val.id + ', \'دستور غذا\'' + ', \'' + val.name + '\'' + ', deleteMenu' + ')"> <i class="fa fa-trash"> حذف </i> </a>' +
                        '</td>' +
                        '</tr>';
                    $("#setMenuList").append(item);
                    $("#LoadingStatus").html(" ");
                });
            }
