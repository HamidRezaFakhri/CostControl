...
var middleUrl = serviceUrl + "ConsumptionUnit/";
...
function trdblclick(id) {
    if (id !== undefined && id >= 0) {
        editEntityModal(id, '/ConsumptionUnit/EditConsumptionUnit/', 'واحد مصرفی');
    }
}
            ...
            if (key === "data") {
                $.each(value, function (i, val) {
                    var item = '<tr id="tr_' + val.id + '" ondblclick="trdblclick(' + val.id + ')">' +
                        ...
                        '<td>' +
                        '<a class="btn btn-sm btn-warning" data-id=\'' + val.id + '\' onClick="editEntityModal(' + val.id + ', \'/ConsumptionUnit/EditConsumptionUnit/\'' + ', \'واحد مصرفی\'' + ')"> <i class="fa fa-edit"> ویرایش </i> </a>' +
                        '<span> | </span>' +
                        '<a class="btn btn-sm btn-danger" data-id=\'' + val.id + '\' onClick="deleteEntityModal(' + val.id + ', \'واحد مصرفی\'' + ', \'' + val.name + '\'' + ', deleteConsumptionUnit' + ')"> <i class="fa fa-trash"> حذف </i> </a>' +
                        '</td>' +
                        '</tr>';
                    $("#setConsumptionUnitList").append(item);
                    $("#LoadingStatus").html(" ");
                });
            }
...
function deleteConsumptionUnit(id) {
