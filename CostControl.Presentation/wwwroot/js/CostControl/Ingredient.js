...
var middleUrl = serviceUrl + "Ingredient/";
...
function trdblclick(id) {
    if (id !== undefined && id >= 0) {
        editEntityModal(id, '/Ingredient/EditIngredient/', 'مواد خوراکی');
    }
}
            ...
            if (key === "data") {
                $.each(value, function (i, val) {
                    var item = '<tr id="tr_' + val.id + '" ondblclick="trdblclick(' + val.id + ')">' +
                        ...
                        '<td>' +
                        '<a class="btn btn-sm btn-warning" data-id=\'' + val.id + '\' onClick="editEntityModal(' + val.id + ', \'/Ingredient/EditIngredient/\'' + ', \'مواد خوراکی\'' + ')"> <i class="fa fa-edit"> ویرایش </i> </a>' +
                        '<span> | </span>' +
                        '<a class="btn btn-sm btn-danger" data-id=\'' + val.id + '\' onClick="deleteEntityModal(' + val.id + ', \'مواد خوراکی\'' + ', \'' + val.name + '\'' + ', deleteIngredient' + ')"> <i class="fa fa-trash"> حذف </i> </a>' +
                        '</td>' +
                        '</tr>';
                    $("#setIngredientList").append(item);
                    $("#LoadingStatus").html(" ");
                });
            }
