...
var middleUrl = serviceUrl + "SalePoint/";
...
function trdblclick(id) {
    if (id !== undefined && id >= 0) {
        editEntityModal(id, '/SalePoint/EditSalePoint/', 'مرکز فروش');
    }
}
            ...
            if (key === "data") {
                $.each(value, function (i, val) {
                    var item = '<tr id="tr_' + val.id + '" ondblclick="trdblclick(' + val.id + ')">' +
                        ...
                        '<td>' +
                        '<a class="btn btn-sm btn-warning" data-id=\'' + val.id + '\' onClick="editEntityModal(' + val.id + ', \'/SalePoint/EditSalePoint/\'' + ', \'مرکز فروش\'' + ')"> <i class="fa fa-edit"> ویرایش </i> </a>' +
                        '<span> | </span>' +
                        '<a class="btn btn-sm btn-danger" data-id=\'' + val.id + '\' onClick="deleteEntityModal(' + val.id + ', \'مرکز فروش\'' + ', \'' + val.name + '\'' + ', deleteSalePoint' + ')"> <i class="fa fa-trash"> حذف </i> </a>' +
                        '</td>' +
                        '</tr>';
                    $("#setSalePointList").append(item);
                    $("#LoadingStatus").html(" ");
                });
            }
